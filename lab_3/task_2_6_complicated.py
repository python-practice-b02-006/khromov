...
def main():
    ...
    while not finished:
        ...
        tree_3 = pygame.Surface((int(W_WIDTH / 3.5), int(W_HEIGHT / 2.5)))
        draw_tree(tree_3)
        screen.blit(tree_3, (int(W_WIDTH / 30), int(W_HEIGHT / 6)))

        tree_4 = pygame.Surface((int(W_WIDTH / 3.5), int(W_HEIGHT / 2.5)))
        draw_tree(tree_4)
        screen.blit(tree_4, (int(W_WIDTH / 1.5), int(W_HEIGHT / 6)))

        panda_1 = pygame.Surface((int(W_WIDTH / 3.2), int(W_HEIGHT / 2.5)))
        draw_panda(panda_1)
        screen.blit(panda_1, (int(1.8 * W_WIDTH / 3), int(W_HEIGHT / 2.5)))

        panda_2 = pygame.Surface((int(W_WIDTH / 6), int(W_HEIGHT / 5)))
        draw_panda(panda_2)
        screen.blit(panda_2, (int(W_WIDTH / 3), int(W_HEIGHT / 2)))
